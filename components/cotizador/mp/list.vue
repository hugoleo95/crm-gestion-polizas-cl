        ...
        <div class="form-sections">
          <div class="table-responsive">
            ...
            >
              ...
              <template #cell(policy_data)="{ value }">
                {{ GetPolice(value) }}
              </template>
              ...
              <template #cell(type_proposal)="{ value }">
                <span :class="['btn-state', getMPStatus(value).class]">
                  <fa-icon
                    :icon="['fas', getMPStatus(value).icon]"
                    class="mr-2"
                  />{{ getMPStatus(value).text }}
                </span>
              </template>
...
import Vista from "@/components/cotizador/mp/view.vue";
export default {
  components: {
    SToolBar,
    Vista,
  ...
  data () {
    return {
      ...
      totalRows: 1,
      quote: {},
      fields: leadsStaticVars.tableFieldsMP,
      option: "list",
      modalShow: false,
      Modal: {
        slider1: true,
  ...
  methods: {
    ...
    async ListPolizasMp (page) {
      this.page = page;
      this.setLoadingLeads(true);
      const data = await this.$ListPolizasMp(page, this.urlParams);
      const Response = data.results;
      console.log(Response);
      this.totalRows = data.count;
      for (let i = 0; i < Response.length; i++) {
        Response[i].persons =
          Response[i].insured_first_name + " " + Response[i].insured_last_name;
    ...
    async GetQuoteWeb (id) {
      const data = await this.$GetMp(id);
      if (data) {
    ...
    newQuote () {
      ...
      if (urlEnv === 'https://api.dev.simplee.cl/api') {
        const url =
          "https://simplee-web.dev.simplee.cl/cotizar_st/seguros/minipyme?step=1&user=" +
        ...
        const url =
          "http://simplee.cl/cotizar_st/seguros/minipyme?step=1&user=" +
          this.$store.state.localStorage.user.id +
          "&user_name=" +
          this.$store.state.localStorage.user.first_name;
        window.location = url;
