<template>
  <b-modal
    ...
    size="lg"
    :title="title"
    body-class="padding-modal bg-primary"
  ...
  >
    <template #modal-header="{}">
      ...
      >
        ...
        <p class="text-primary">Manten presionado para arrastrar el modal</p>
      </div>
    </template>
...
export default {
  ...
  created () {
    if (this.dataproduct) {
      this.productsdata = this.dataproduct
    }
    ...
    saveLead () {
      ...
      /* if (this.productsdata.tipo_de_pago === 'aviso_de_cobranza'
        ...
        if (!this.productsdata.payment_day
          ...
          this.$
          this.BotonGuardar = falsestore.commit('setAlertMessage', { error: 'Si el tipo de pago es aviso de cobranza, debe ingresar los dias de cobro' })
          return
        }
      }
      */
      if (this.productsdata.product_type === 'poliza') {
        ...
        this.productsdata.endorsement_info = null
      }
      ...
      if (this.version === 'CL' && this.productsdata.product_type !== 'endoso') {
        if (
