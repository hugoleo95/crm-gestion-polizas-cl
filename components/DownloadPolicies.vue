    ...
    >
      ...
      <b-dropdown-form class="w-full">
        ...
        <div v-if="step === 3">
          ...
          <label class="mb-2">Fecha {{ startDate }} a {{ endDate }}</label>
          <vue-json-to-csv
...
export default {
  ...
  computed: {
    ...
    fileTitle () {
      return `Polizas (${this.startDate} - ${this.endDate})`
    },
    computedPages () {
  ...
  methods: {
    ...
    async consultPoliciesPerDate () {
      this.loading = true
      try {
        const res = await this.$simpleeApi.get('/product/', { params: { start: this.startDate, end: this.endDate } })
    ...
    async consultPoliciesPerDateAndPages () {
      ...
      for (let i = 0; i < this.pages; i++) {
        const res = await this.$simpleeApi.get('/product/', { params: { start: this.startDate, end: this.endDate, page: i + 1 } })
