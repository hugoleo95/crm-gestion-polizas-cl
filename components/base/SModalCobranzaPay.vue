<template>
  <div>
    <b-modal
      ...
      ref="leadModal"
      v-model="localShow"
      modal-class="draggable-modal size-max"
      size="xl"
      ...
      <div class="d-flex w-100 flex-column align-items-center">
        <div class="text-center w-100 pt-3">
          <InputMassive
            ...
            urldocument="/formatocargacobranzapay.xlsx"
        ...
        <div
          v-if="Statuscarga !== 'leido' && Statuscarga !== 'finalizado'"
          class="w-100 mt-3"
        >
          <span>Consejos:<br>-Por favor el numero de poliza tiene que ser el mismo que esta cargado en el Simplee Teams<br> -Los montos de las cuotas se debe usar punto para indicar los decimales, eje: 28.5<br>
            -Las fechas tienen que tener un formato eje: 2024-12-01<br>-Los numeros de telefono tienen que ir con el indicativo de pais, eje: 56983276687</span>
        ...
        >
          ...
          >
            ...
            <template #cell(datacompleted)="{ value }">
              <input
                type="date"
                :value="value.payment_date"
                @change="UpdateFecha($event.target.value,value.nro_poliza)"
              >
            </template>
          </b-table>
        </div>
        <div
          ...
          class="w-100 mt-3"
        >
          <span>Se registraron las cuotas exitosamente</span>
        </div>
        <div
  ...
  <script>
import InputMassive from '@/components/InputMassiveCobranzaPay'
  ...
  methods: {
    ...
    async saveLead () {
      ...
      for (let i = 0; i < PolizasEncontradas.length; i++) {
        const existe = this.CuotasExistentes.find(key => key.paid_dues === PolizasEncontradas[i].paid_dues && key.nro_poliza === (PolizasEncontradas[i].nro_poliza).toString())
        if (existe) {
          console.log(this.CuotasExistentes, 'aquii')
          console.log('existe', PolizasEncontradas[i].paid_dues, PolizasEncontradas[i].nro_poliza)
        } else {
          console.log(this.CuotasExistentes, 'aquii2')
          console.log('no existe', PolizasEncontradas[i].paid_dues, PolizasEncontradas[i].nro_poliza)
          delete PolizasEncontradas[i].datacompleted
          CuotasFinals.push(PolizasEncontradas[i])
        }
        
      }
      console.log('aquii en total', CuotasFinals)
      try {
        const { data } = await this.$simpleeApi.post(
          '/cobranza/bulk_upload_pd/', { payload: { payload: CuotasFinals } }
        )
        this.CobrosSatisfactorios = CuotasFinals.length
        console.log(data)
      } catch (error) {
        console.log(error)
      }
      ...
      this.$emit('carga-cuotas')
      this.$emit('carga-cuotas-totales')
      // this.$emit('save-lead')
    },
    ReverCarga () {
    ...
    UpdateFecha (value, nro_poliza) {
      const index = this.PolizasEncontradas.findIndex((x) => x.nro_poliza === nro_poliza);
      this.PolizasEncontradas[index].payment_date = value
    },
    addCobranzas (register) {
      register.state = 'PENDING'
      register.phone = '+' + register.phone
      const Datenew = moment('1900-01-01')
      register.payment_date = Datenew.add(register.payment_date - 2, 'days').format("YYYY-MM-DD");
      register.datacompleted = register
