  ...
  >
    ...
    >
      <section class="ftco-section">
                ...
                <table class="table table-hover table-striped mb-0">
                  <thead>
                    <tr>
                      ...
                      <th class="text-center">Ejecutiva/o</th>
                      <th></th>
                    </tr>
                  </thead>
                  <tbody>
                    ...
                    >
                      <th
                        class="text-center"
                        scope="row"
                      >{{ re.name }} {{ re.last_name }}</th>
                      <td class="text-center">{{ re.email }}</td>
                      <td class="text-center"> {{ re.razon_social }}</td>
                      <td class="text-center">{{ getLeadUser(re.user) }}</td>
                      <td class="text-center">
                        <div class="d-flex justify-content-center">
                          <button
                            class="circle mr-2 bg-white shadow-sm border p-3"
                            @click="ModificarContacto(re.id)"
                          >
                            <fa-icon
                              :icon="['fas', 'thumbtack']"
                              class="text-primary"
        ...
        >
          <span>
            Por favor ingrese el nombre completo, correo completo o razon social completo del contacto en el campo de busqueda
          </span>
        </div>
      </section>
    </div>
        ...
        <div class="form-row w-100 d-flex justify-content-center">
          <div class="form-group-relation">
            <label
              class="text-base"
              for="name"
            >Por favor ingrese la relación con el contacto :</label>
            <input
              id="email"
              v-model="dates.type"
              placeholder=""
...
export default {
  ...
  methods: {
    debouncedSearch: _.debounce(function (e) {
      this.getContacto(e.target.value)
    }, 750),
    async getContacto (dato) {
      this.loading = true
      const params = {
        search: dato
      }
      ...
      try {
        const {
          data: { results, count }
        } = await this.$simpleeApi.get('/lead/', { params: { page, ...params } })
        this.nContactos = count
        this.Contactos = results
        if (!dato) {
          this.nContactos = 0
          this.Contactos = {}
        }
    ...
    getLeadUser (user) {
      if (user) {
        const { firstname, email } = this.$store.state.leads.users.find(({ id }) => user === id)
    ...
    RelationLead () {
      this.$emit('UpdateContact', this.dates)
    },
    newItem () {
    ...
    ModificarContacto (lead) {
      this.$bvModal.msgBoxConfirm('¿Esta seguro de relacionar con este contacto?', {
        title: 'Confirmar',
        titleClass: 'mdtitle',
        size: 'md',
        buttonSize: 'md',
...
.form-row .form-group-relation {
  display: flex;
  flex-direction: column;
  width: 50%;
  padding: 12px 15px;
}
.form-col .form-group-relation {
  width: 100%;
  padding-left: 20px;
  padding-right: 20px;
  padding-top: 12px;
