<template>
  <div class="dropzonepa">
    ...
    <div v-if="type === 'product'" class="mt-3">Descargue el formato excel
      <a
        :href="urldocument"
        download
      >
  ...
  methods: {
    ...
    productMethod () {
      if (this.dropzoneFile[0]) {
        ...
        if (!fileType) {
          alert('Hubo un error al cargar el archivo, el formato tiene que ser de excel, intente nuevamente.')
        } else {
          ...
          reader.onload = (e) => {
            ...
            for (let i = 0; i < rowObject.length; i++) {
              if (rowObject[i].user) {
                let register = {}
                register = {
                  line: rowObject[i].line,
                  name: rowObject[i].name,
                  company: rowObject[i].company,
                  prima: rowObject[i].prima,
                  description: rowObject[i].description,
                  comision: rowObject[i].comision,
                  nro_poliza: rowObject[i].nro_poliza,
                  emited_date: rowObject[i].emited_date,
                  fecha_inicio_vigencia: rowObject[i].fecha_inicio_vigencia,
                  fecha_termino_vigencia: rowObject[i].fecha_termino_vigencia,
                  tipo_de_pago: rowObject[i].tipo_de_pago,
                  user: rowObject[i].user
                }
