            ...
            <div class="signup-form margin-text p-4 item">
              <div>
                <div class="w-100 d-flex flex-row">
                  <div class="w-50"><label class="text-label"> Otros datos de la Cobranza </label></div>
                  <div>
                    <a
                      class="pagos-btn"
                      @click="DeletePay(DataMadre.id)"
                    >
                      <fa-icon
                        :icon="['fas', 'trash']"
                ...
                <div class="form-row mtrow">
                  <div class="form-group-request w-50">
                    <span class="text-base w-100 d-flex flex-row">
                      <strong class="pr-1">
                        Compañia:</strong> {{DataMadre.company}}
                    </span>
                  </div>
                  <div class="form-group-request w-50">
                    <span class="text-base w-100 d-flex flex-row">
                      <strong class="pr-1">
                        Ejecutivo:</strong> {{DataMadre.executive}}
                    </span>
                  </div>
                </div>
                <div class="form-row mtrow">
                  <div class="form-group-request w-50">
                    <span class="text-base w-100 d-flex flex-row">
                      <strong class="pr-1">
                        Telefono:</strong> {{DataMadre.phone}}
                    </span>
                  </div>
                  <div class="form-group-request w-50">
                    <span class="text-base w-100 d-flex flex-row">
                      <strong class="pr-1">
                        Correo:</strong> {{DataMadre.email}}
                    </span>
                  </div>
                </div>
...
export default {
  ...
  methods: {
    ...
    async getCobranza () {
      const item = {
        physical_debtors: this.idCuota
      }
      try {
        const { data } = await this.$simpleeApi.get('cobranza/physical_debtors_due/', { params: item })
        const results = data.results
        // console.log(results)
        this.CuotasObject = results
      } catch (error) {
        console.log(error)
    ...
    DeletePay (data) {
      this.$bvModal
        .msgBoxConfirm('¿Esta seguro de eliminar este pago de esta cuota?', {
          title: 'Confirmar',
          titleClass: 'mdtitle',
          size: 'md',
          buttonSize: 'md',
        ...
        .then((value) => {
          if (value) {
            setTimeout(() => this.DeletePayEnd(data), 500)
          }
        })
    },
    async DeletePayEnd (id) {
      try {
        await this.$simpleeApi.delete(`cobranza/physical_debtors/${id}/`)
        this.$store.commit('setAlertMessage', { success: 'El pago fue borrado exitosamente' })
        this.$emit('BackList')
      } catch (error) {
        console.log({ error })
        this.$store.commit('setAlertMessage', error.response.data)
    ...
    PayCobro (data) {
      this.$bvModal
        ...
        .then((value) => {
          if (value) {
            setTimeout(() => this.UpdateCuotaHija(data, 'PAID'), 500)
          }
        })
    },
    async UpdateCuotaHija (item, state) {
      const payload = {
        id: item.physical_debtors,
        due: item.due,
        state: state
      }
      try {
