  ...
  >
    <b-card-body class="d-flex flex-column" :class="cargardata ? 'cardDespues' : 'cardAntesCarga'">
      ...
      <template v-else>
        ...
        <div class="d-flex flex-column flex-fill justify-content-start align-items-left">
...
export default {
  ...
  data () {
    return {
      ...
      loading: false,
      chart: {
        labels: ['Prima', 'Comision', 'Comision Simple'],
        datasets: [
            {
                backgroundColor: ["#00cc99", "#027afa", "#7673ff"],
                data: []
            }
        ]
      },
  ...
  methods: {
    ...
    async getChart () {
      ...
      try {
        ...
        for (let i = 1; i <= 3; i++) {
          let montoMostrar = this.formtNum(data[i])
          montoMostrar = parseFloat(montoMostrar.replace(/\./g, '').replace(',', '.'));
          this.chart.datasets[0]['data'].push(montoMostrar);
        }
      } catch (error) {
